...
# --- Enhanced Download/Upload Functions ---
async def download_file_with_speed(client, file_id, download_path, callback):
    """Download file with enhanced speed using threading"""
...
async def upload_file_with_speed(client, file_type, file_path, upload_params):
    """Upload file with enhanced speed using threading"""
    with concurrent.futures.ThreadPoolExecutor() as executor:
        loop = asyncio.get_event_loop()
        ...
        if file_type == "document":
            ...
            media = await client.get_messages(upload_params['chat_id'], upload_params.get('message_id', 0))
            video_meta = media.video if media else None
            upload_params_copy = upload_params.copy()
            upload_params_copy.pop('message_id', None)
            
            return await loop.run_in_executor(
                executor, 
                lambda: client.send_video(
                    video=file_path,
                    duration=video_meta.duration if video_meta else None,
...
async def status_handler(client: Client, message: Message):
    ...
    status_text = (
        ...
        f"• **Bot connected:** ✅\n"
        f"• **Web server:** ✅\n"
        f"• **Speed Mode:** Extreme Performance\n"
    )
...
async def process_file(client: Client, message: Message):
    ...
    try:
        ...
        await download_file_with_speed(client, task["file_id"], download_path, download_callback)
        ...
        # Use os.rename for faster file operations
        ...
        await upload_file_with_speed(client, file_type, new_file_path, upload_params)
