...
def authorized_users_only(func):
    def wrapper(client, message):
        if message.from_user.id in AUTH_USERS:
            ...
            message.reply_text("Sorry, you are not authorized to use this bot.")
    return wrapper
...
async def start_command(client, message):
    ...
    await message.reply_text(
        ...
        "I can rename any file you send me. Just send a file and then reply with the new name.",
...
async def rename_file(client, message: Message):
    ...
    if not (original_file_message.document or original_file_message.video or original_file_message.audio):
        await message.reply_text("Something went wrong. Please send the file again.")
        return
    ...
    if file_type:
        try:
            ...
            # For videos and audio, we can try to copy metadata
            if hasattr(file_info, 'duration'):
                kwargs['duration'] = file_info.duration
            if hasattr(file_info, 'width'):
                kwargs['width'] = file_info.width
            if hasattr(file_info, 'height'):
                kwargs['height'] = file_info.height
            if hasattr(file_info, 'performer'):
                kwargs['performer'] = file_info.performer
            if hasattr(file_info, 'title'):
                kwargs['title'] = file_info.title
